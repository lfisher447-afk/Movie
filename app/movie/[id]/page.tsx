...
// FIX 1: Removed unused imports — Server, ShieldAlert, ShieldCheck, ShieldX
import { Power, Check, Plus, Monitor, Activity, Users, PlayCircle, Radio } from 'lucide-react';
...
export default function MediaDetail({ params, searchParams }: { params: { id: string }, searchParams: { type?: string, room?: string } }) {
  ...
  // FIX 2: Removed unused local adBlockMode state — it duplicates the one in useStore
  // and was never read or passed anywhere in this component.
  ...
  // FIX 3: videoRef is only forwarded to WatchPartyOverlay, which uses it
  // for video sync. The relay branch was using a raw <iframe> instead of
  // NexusPlayer (see FIX 5), so videoRef typing is corrected to match only
  // what WatchPartyOverlay actually needs: a plain ref object.
  ...
  // FIX 4: Reset currentNode to 0 whenever playerMode switches to relay,
  // preventing a stale node index pointing to an out-of-bounds source.
  useEffect(() => {
    if (playerMode === 'relay') setCurrentNode(0);
  }, [playerMode]);
  ...
  if (!isMounted || !media) return (
    <div className="h-screen bg-surface flex items-center justify-center">
      <Activity className="animate-spin text-brand w-16 h-16" />
    </div>
  );
  ...
  // FIX 5: Build relay sources array for NexusPlayer so the relay branch
  // uses the same NexusPlayer component (HLS engine, ambient mode, controls)
  // instead of a bare <iframe> that bypassed all player features and had no
  // error handling, quality switching, or analytics.
  const relaySources = useMemo(() => {
    if (streams?.sources?.length > 0) {
      return streams.sources.map((s: any) => ({
        serverId: s.id || s.name,
        name: s.name,
        url: s.url,
        isDirect: !!s.isDirect,
        tier: s.tier ?? 2,
          ...
          <div className="flex-1 space-y-6 flex flex-col h-full z-10 transition-all w-full">
            {cinema ? (
              <div className="flex-1 flex flex-col relative w-full shadow-2xl rounded-[2rem] border border-white/5 overflow-hidden group/player bg-[#0a0a0f]">
                {/* FIX 5 (continued): Both branches now render NexusPlayer */}
                {playerMode === 'native' ? (
                  nativeSources.length > 0 ? (
                    <NexusPlayer
                      ...
                      mediaId={params.id}
                      title={media.title || media.name}
                      poster={`https://image.tmdb.org/t/p/w1280${media.backdrop_path}`}
                    />
                  ) : (
                  ...
                  relaySources.length > 0 ? (
                    <NexusPlayer
                      ref={videoRef}
                      sources={[relaySources[currentNode]]}
                      mediaId={params.id}
                      title={media.title || media.name}
                      poster={`https://image.tmdb.org/t/p/w1280${media.backdrop_path}`}
                    />
                ...
                <button
                  onClick={() => {
                    setCinema(false);
                    window.history.pushState({}, '', window.location.pathname + `?type=${type}`);
                    setRoomCode(null);
                  }}
                  className="absolute top-6 right-6 p-4 bg-black/60 hover:bg-brand rounded-2xl opacity-0 group-hover/player:opacity-100 z-[150] font-bold text-white transition-all transform hover:scale-105"
                >
              ...
              <div
                className="w-full aspect-video rounded-[2rem] overflow-hidden relative cursor-pointer group"
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/40 group-hover:bg-black/20 transition-colors flex items-center justify-center">
                  <div className="w-24 h-24 bg-brand/90 text-white rounded-full flex items-center justify-center pl-2 shadow-[0_0_50px_rgba(229,9,20,0.8)]">
            ...
            <div className={`flex flex-col gap-4 glass-panel p-6 rounded-3xl w-full border-brand/5 shadow-2xl transition-all ${cinema ? 'mt-4 border-white/10 bg-white/5' : 'animate-reveal'}`}>
              <div className="flex flex-col lg:flex-row lg:items-center justify-between w-full border-b border-white/10 pb-4 gap-4">
                <div className="flex items-center gap-3 bg-black/40 p-1.5 rounded-2xl border border-white/10">
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-[10px] font-black tracking-widest transition-all uppercase flex items-center gap-2 ${playerMode === 'native' ? 'bg-brand text-white' : 'text-gray-500 hover:text-white'}`}
                  >
                    <Activity className="w-3 h-3" /> NATIVE ENGINE
                  </button>
                  <button
                    onClick={() => setPlayerMode('relay')}
                    className={`px-4 py-2 rounded-xl text-[10px] font-black tracking-widest transition-all uppercase flex items-center gap-2 ${playerMode === 'relay' ? 'bg-brand text-white' : 'text-gray-500 hover:text-white'}`}
                  >
                  ...
                  <div className="flex items-center gap-4 p-2 bg-black/40 rounded-2xl border border-white/10">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="bg-black/60 text-white text-xs px-3 py-1.5 rounded-xl border border-white/5 w-20 text-center outline-none focus:border-brand"
                        placeholder="Season"
                      />
                      <input
                        type="number"
              ...
              {playerMode === 'relay' && (
                <div className="flex flex-wrap gap-3 w-full max-h-[160px] overflow-y-auto custom-scrollbar pr-2 pb-2">
                  {/* FIX 6: Guard against undefined streams.sources before mapping */}
                  {(streams?.sources ?? []).map((s: any, i: number) => (
                    <button
                      key={i}
                      onClick={() => setCurrentNode(i)}
                      className={`px-5 py-3 rounded-2xl text-[9px] font-black tracking-[0.2em] transition-all flex flex-shrink-0 items-center gap-2 ${currentNode === i ? 'bg-brand text-white shadow-brand-glow border-brand' : 'bg-black/60 text-gray-400 hover:text-white hover:bg-white/10 border-white/5'} border`}
                    >
    ...
    <div className="w-full xl:w-[480px] space-y-8 flex-shrink-0 animate-reveal" style={{ animationDelay: '0.2s' }}>
      <div className="glass-panel p-10 space-y-8 border-brand/10 shadow-[0_30px_80px_-15px_rgba(0,0,0,0.8)] relative overflow-hidden">
        <div className="relative z-10">
          <h1 className="font-nexus text-5xl md:text-7xl leading-[0.85] text-white drop-shadow-2xl mb-5 tracking-wide">
            {media.title || media.name}
          </h1>
        </div>
        <p className="text-gray-300 leading-relaxed text-sm font-medium opacity-90 line-clamp-6 drop-shadow-md relative z-10">
          {media.overview}
        </p>
        <div className="grid grid-cols-2 gap-4 pt-6 border-t border-white/10 relative z-10">
          <button
            onClick={() => {
              ...
              toast.success(isSaved ? 'VAULT PURGED' : 'VAULT SECURED');
            }}
          ...
          >
            ...
            {isSaved ? 'STORED' : 'SECURE'}
          </button>
          <button
            onClick={() => setCinema(true)}
            className="btn-nexus bg-black/60 h-16 border border-white/10 hover:bg-white/10 text-white hover:border-brand/40 shadow-inner"
          >
            <Monitor className="w-5 h-5 text-brand" /> CINEMA
          </button>
        </div>
        <button
          onClick={initializeWatchParty}
          className="btn-nexus w-full bg-gradient-to-r from-brand to-red-900 border border-brand/50 text-white h-16 group relative overflow-hidden shadow-brand-glow z-10"
        >
          <Users className="w-5 h-5 relative z-10" />
          <span className="relative z-10">INITIATE WATCH PARTY</span>
        </button>
      </div>
    </div>
  );
...
// FIX 7: Star component was defined but never used anywhere in the file.
// Kept here but flagged — remove it or wire it up to media.vote_average display.
