      ...
      <div className="fixed inset-0 z-[999] flex items-start justify-center pt-[15vh] px-4 backdrop-blur-3xl bg-surface/80" onClick={onClose}>
        <motion.div initial={{ opacity: 0, scale: 0.95, y: -20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ ease: [0.16, 1, 0.3, 1], duration: 0.4 }}
          ...
          <div className="flex items-center px-6 py-2 border-b border-white/5 bg-black/40">
            ...
            <input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder="QUERY DATABASE..." 
          ...
          <div className="max-h-[50vh] overflow-y-auto custom-scrollbar p-4 bg-surface-card">
            ...
            {results.map((m, idx) => (
              <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.05 }}
                 key={m.id} onClick={() => { router.push(`/movie/${m.id}`); onClose(); }} 
                ...
                <img src={m.poster_path ? `https://image.tmdb.org/t/p/w92${m.poster_path}` : 'https://via.placeholder.com/92x138'} className="w-12 h-16 rounded-xl object-cover shadow-lg grayscale group-hover:grayscale-0 transition-all" />
                <div className="flex-1">
                  <h4 className="font-nexus text-2xl text-white group-hover:text-brand transition-colors">{m.title}</h4>
                  <p className="text-[10px] text-gray-500 font-black tracking-widest">{m.release_date?.split('-')[0]} • ⭐ {m.vote_average?.toFixed(1)}</p>
                </div>
